...
class MainTabLayouts {
    ...
    func popularNowSection() -> NSCollectionLayoutSection {
        ...
        let groupSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1), heightDimension: isWidthBigger() ? .fractionalWidth(0.4) : .absolute(200))
        ...
        section.orthogonalScrollingBehavior = isWidthBigger() ? (flag ? .continuousGroupLeadingBoundary : .groupPaging) : (flag ? .continuousGroupLeadingBoundary : .paging)
    ...
    func nowInTheatresSection() -> NSCollectionLayoutSection {
        let itemSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1), heightDimension: .fractionalHeight(isWidthBigger() ? 1.0 : 1.0))
        let item = NSCollectionLayoutItem(layoutSize: itemSize)
        
        let groupSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(isWidthBigger() ? 0.25 : 0.7), heightDimension: .fractionalWidth(isWidthBigger() ? 0.35 : 1.0 ))
        let group = NSCollectionLayoutGroup.horizontal(layoutSize: groupSize, subitems: [item])
        group.contentInsets = NSDirectionalEdgeInsets(top: 0, leading: 0, bottom: 0, trailing: 8)
        
        let section = NSCollectionLayoutSection(group: group)
        section.contentInsets = NSDirectionalEdgeInsets(top: 10, leading: 0, bottom: 8, trailing: 0)
        section.orthogonalScrollingBehavior = isWidthBigger() ? .groupPaging : .groupPagingCentered
